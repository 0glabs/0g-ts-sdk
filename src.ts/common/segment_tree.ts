...
export function pushdown(node: SegmentTreeNode): void {
	if (node.childs === null) {
		node.childs = []
		for (let i = 0; i < 2; i += 1) {
			node.childs.push({
...
export function insert(node: SegmentTreeNode, numShard: number, shardId: number, expectedReplica: number): boolean {
